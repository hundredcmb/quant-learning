...
| `shenwan_industry/docs/` | 模块文档：`interface_notes.md`（Tushare 接口交互明细）、`known_issues.md`（已知边界与易错点）、`roadmap.md`（未来规划）、`sync_progress.md`（官方算法同步进度）、`financial_indicators.md`（财务指标算法：PE/PB/净利润同比/ROE）、`Shenwan_Index_Series_Algorithm_Text.md`（申万官方指数算法纯文字版，**只读禁止修改**） |
