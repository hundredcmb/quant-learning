...
- **申万行业分析**（`shenwan_industry/`）：申万 2021 三级行业分类树 + 单日/区间行业涨幅榜（等权 / 流通市值加权）+ 单日榜行业财务指标（**PE**、**净利润同比** 与 **ROE**（净利润口径四选一：归母/扣非 × TTM/动态，Web 下拉切换、默认归母-TTM；同比扭亏/转亏/持续亏损类别化；ROE 为加权平均算法、随"ROE算法"下拉选择）、**PB** 归母普通股股东权益，PE/PB 各自由流通 / 总市值两种合成口径，计算方法见 `shenwan_industry/docs/financial_indicators.md`），Web 可视化界面。**已彻底脱离 vnpy**，任意 Python 3.10+ 环境即可运行
...
- 单日排行 / 区间排行（等权 / 流通市值加权，L1/L2/L3 三级；单日榜含行业 PE/ROE/净利润同比列，PE 随加权方式显示对应口径、随"净利润口径"下拉切换归母/扣非 × TTM/动态四档（等权为空），ROE 与净利润同比无市值维度等权也显示）
- 行业成分股子表
