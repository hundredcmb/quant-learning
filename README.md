...
- 单日排行 / 区间排行（等权 / 流通市值加权，L1/L2/L3 三级；两榜均含行业 PE/PB/ROE/股息率/净利润同比列——单日榜为当日时点、区间榜（官方逐日链式，Web 默认）为**区间末交易日时点值**（表头注明），PE 随加权方式显示对应口径、随"净利润口径"下拉切换归母/扣非 × TTM/动态四档（等权为空），ROE 与股息率、净利润同比无市值维度等权也显示）
- 行业成分股子表
