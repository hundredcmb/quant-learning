...
> **Tushare 积分要求与启动探测**：`top10_holders` **至少 2000 积分**、ETF 日线 `fund_daily` **至少 5000 积分**。各脚本启动时会先用一条固定历史数据真实调用对应门槛接口探测权限：股票脚本低于 2000、ETF 脚本低于 5000 时直接报错退出（仅网络异常时提示跳过检查并继续）。仓库自带的原始数据缓存 `holders/stock/tushare_top10_holders_raw.json` 针对样本池 **中证 800 + 中证 1000 成分股**（约 1800 只），完整覆盖 **2025 年年报（`20251231`）及以后**（如 `20260331`）。
