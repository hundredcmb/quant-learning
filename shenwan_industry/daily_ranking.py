...
if __name__ == "__main__":
    ...
    (l1_rank_list_ew, l2_rank_list_ew, l3_rank_list_ew), \
        ...
        (l1_rank_list_twr, l2_rank_list_twr, l3_rank_list_twr), timings, pe_data = run_daily_ranking(
    ...
    print_timing(
        [
            ...
            ("财务指标", [("PE-TTM 数据获取 fina_indicator_vip", timings.get("fina_fetch", 0.0)),
    ...
    for industry_level in industry_levels:
        ...
        print(f"总市值加权涨幅(官方价格)|总市值·分红再投资涨幅|自由流通市值加权涨幅(官方价格)|自由流通·分红再投资涨幅|等权涨幅(官方价格)|等权·分红再投资涨幅|PE-TTM(自由流通)|PE-TTM(总市值)|行业名称|成分股数量 成分股列表")
        for index_ts_code, index_pct_chg, stock_count in rank_list:
            ...
            # PE 列: 键缺失 -> "—"(无数据/计算失败); 值为 None -> "亏损"(行业扣非TTM合计<=0)
            def _fmt_pe(pe_for_level: dict[str, float | None], code: str) -> str:
                if code not in pe_for_level:
                    return "—"
                value = pe_for_level[code]
                if value is None:
                    return "亏损"
                return f"{value:.2f}"
            ...
            print(f"{'+' if index_pct_chg_tw >= 0 else ''}{index_pct_chg_tw:.2f}%|" +
                  ...
                  f"{_fmt_pe(pe_free_for_level, index_ts_code)}|" +
                  f"{_fmt_pe(pe_total_for_level, index_ts_code)}|" +
