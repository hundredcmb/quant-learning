...
- 股息率(列名"股息率")双口径, 规则栈与接口实测见 docs/financial_indicators.md 第 7 节:
  ...
  * **静态股息率口径** = 最近完整分红年度(锚)的级联总额; 锚 = 年度事件(end_date=*1231)**有实施或
    有预案**的最近财年(预案先行, 每股锚切换提前到 3~4 月; 分批除息实测为零)
  * **TTM估算股息率口径(Web 默认)** = **进行中财年 N 的估算, 随每期利润报告刷新**(目标财年 =
...
def compute_dividend_dps(
    ...
    est_map: "TTM估算股息率"口径(Web 默认)——进行中财年的宣告优先/外推补位值
    static_map: "静态股息率"口径——最近完整分红年度的级联总额
