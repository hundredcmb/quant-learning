...
- **数据**：`fina_indicator_vip` 按报告期全市场批拉，**一次请求同取** `profit_dedt`（归属母公司扣非净利润，**年初至今累计值**，元）与 `bps`（每股净资产，**报告期末时点值**，元），字段 `ann_date`=公告日；limit 参数生效，offset/limit=5999 分页循环（全量单次可到 8808 行，见 interface_notes）；**同股票同报告期有重复行（含 NaN 行），去重为字段级各自取最后非空**（实测 bps 两行均有效但值不同 56.7800/56.7751，不能整行丢弃）；fields 指定不存在的字段名**静默忽略**，须 `getattr` 防御；每接口独立节流（7.5/s）
