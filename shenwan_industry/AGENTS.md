...
- 当前实现**没有显式限流/休眠**，全市场分页为顺序循环；改动时注意 Tushare 按积分的频率限制，不要引入并发放大调用量
...
10. 复权字段单位（已实测验证）：tushare `dividend` 的 `stk_div` 为每股送转股数（10送10 → 1.0）、`cash_div` 为每股派现元（10派10 → 1.0）；`adj_factor` 与 `daily.pre_close` 同属**除权参考价（价格）口径**，比值收益 = close/除权参考价 − 1，现金分红不计入收益；`price_adjust.py` 的“送转除、派现减”静态前复权与其一致
